...
function isTacoAttack(Player)
    return Player.PlayerData.job.name == 'tacoattack'
end
...
if ox_inv then
    for i = 1, #Config.Zones do
        ...
        elseif v.type == 'shop' then
            exports.ox_inventory:RegisterShop('Ingredients - Taco Attack', {
                name = 'Ingredients - Taco Attack',
                inventory = {
                    { name = 'tacofilling', price = 0 },
                    { name = 'burritofilling', price = 0 },
                    { name = 'beans', price = 0 },
                    { name = 'salsa', price = 0 },
                    { name = 'wheatflour', price = 0 },
                    { name = 'water_bottle', price = 0 },
                    { name = 'sugarpacket', price = 0 },
                    { name = 'cheesesauce', price = 0 },
                },
            })
        end
    end
    for k,v in pairs(Config.HungerFill) do
        exports(k, function(event, item, inventory, slot, data)
            if event == 'usingItem' then
                TriggerClientEvent('stag_tacoattack:client:Eat', inventory.id, k, slot, v.emote)
            end
        end)
    end
    for k,v in pairs(Config.ThirstFill) do
        exports(k, function(event, item, inventory, slot, data)
            if event == 'usingItem' then
                TriggerClientEvent('stag_tacoattack:client:Drink', inventory.id, k, slot, v.emote)
            end
        end)
    end
else
    for k,v in pairs(Config.HungerFill) do
        QBCore.Functions.CreateUseableItem(k, function(source, item)
            TriggerClientEvent('stag_tacoattack:client:Eat', source, k, item.slot, v.emote)
        end)
    end
    for k,v in pairs(Config.ThirstFill) do
        QBCore.Functions.CreateUseableItem(k, function(source, item)
            TriggerClientEvent('stag_tacoattack:client:Drink', source, k, item.slot, v.emote)
        end)
    end
end